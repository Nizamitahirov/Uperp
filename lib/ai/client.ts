...
export async function generateProductDescription(attributes: Record<string, unknown>): Promise<{ az: string; en: string }> {
  const res = await fetch('/api/ai/generate', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ task: 'product_description', attributes }),
  });
  if (!res.ok) {
